...
class UserDAO:
  def addNewUser2(self, user_data):
    ...
    query = '''INSERT INTO users (first_name, last_name, date_of_birth, gender, phone_number, email_address, password, is_premium) VALUES(?,?,?,?,?,?,?,?)'''
  ...
  def updateUsers2(self, user_data):
    conn, cursor = db_connect()
    query = '''UPDATE users SET first_name=?, last_name=?, date_of_birth=?, gender=?, phone_number=?, email_address=?, password=?, is_premium=? WHERE is_deleted=0 and user_id=?'''
  ...
  def getUserById2(self, user_id):
    conn, cursor = db_connect()
    query = 'SELECT user_id, first_name, last_name, date_of_birth, gender, phone_number, email_address, is_premium FROM users WHERE is_deleted=0 and user_id=?'
